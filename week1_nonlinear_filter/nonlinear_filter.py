...
def calculateRangeKernel(pixel_center,pixel,sigmaColor):
	return np.exp(-((int(pixel_center)-int(pixel))**2/(2*int(sigmaColor)**2)))
...
def bilateralFilterWindow(window,sigmaColor,sigmaSpace):
	d = window.shape[0]
	center_pixel = window[int(d/2),int(d/2)]
	w_sum = 0
	numerator = 0
	for i in range(d):
		for j in range(d):
			domain_kernel = calculateDomainKernel(d/2,d/2,i,j,sigmaSpace)
			range_kernel = calculateRangeKernel(center_pixel,window[i,j],sigmaColor)
...
if __name__ == '__main__':
	...
	filtered_img_my = calculateRunTime(bilateralFilterMy,source_image,9,75,75)
	print('\n')
